...
function toArchetypeDisplayName(ref: string): string {
  if (!ref.startsWith('archetype:')) return ref;
  return ref.slice('archetype:'.length).replace(/([a-z])([A-Z])/g, '$1 $2');
}
...
export default function FeaturesIndexPage() {
  ...
  const filteredFeatures = features.filter(feature =>
    ...
    feature.description.some(d => d.toLowerCase().includes(search.toLowerCase())) ||
    feature.prerequisites.some(p => p.toLowerCase().includes(search.toLowerCase())) ||
    (feature.archetypes || []).some(a => a.toLowerCase().includes(search.toLowerCase()))
  );
  ...
  return (
    <div className="space-y-8">
      <Breadcrumb items={[{ label: 'Rules', href: '/rules' }, { label: 'Features' }]} />
              ...
              <div className="mb-4 pl-9">
                ...
                <div className="flex flex-wrap gap-2">
                  {feature.archetypes.map((archetype, idx) => {
                    ...
                    const slug = archetype.startsWith('archetype:')
                      ? idToSlug(archetype)
                      : null;
                    ...
                    return slug ? (
                      ...
                      <span key={`${archetype}-${idx}`} className="text-xs px-2 py-1 rounded border border-zinc-700 text-zinc-300">
                        {label}
                      </span>
                    );
                  })}
