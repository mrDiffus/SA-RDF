...
const OUTPUT_FILE = path.join(DATA_DIR, 'resource-index.json');
...
function extractResources(filePath, relativeFilePath) {
  try {
    ...
    // Accept files that declare a type or label, using either JSON-LD @type or compact 'type'/'label' aliases
    const hasType = data['@type'] || data['type'];
    const hasLabel = data['rdfs:label'] || data['label'];
    if (!data['@graph'] && !hasType && !hasLabel) {
      return [];
    }
...
function processResource(item, filePath) {
  ...
  // Resolve compact aliases used in JSON-LD @context (e.g. "type" -> "@type", "label" -> "rdfs:label")
  const rawType = item['@type'] || item['type'];
  const resolvedType = Array.isArray(rawType) ? rawType[0] : rawType;

  const resource = {
    file: filePath,
    '@type': resolvedType || 'unknown',
    'rdfs:label': item['rdfs:label'] || item['label'] || item.name || item.id || '(unlabeled)',
    '@id': item['@id'] || null,
  ...
  if (item['sa:description']) {
    const desc = item['sa:description'];
    const text = Array.isArray(desc) ? desc[0] : desc;
    if (typeof text === 'string') resource['sa:description'] = text.substring(0, 100) + '...';
  }
  ...
  if (item['description'] && typeof item['description'] === 'string') {
    resource['description'] = item['description'].substring(0, 120) + '...';
  }
...
try {
  buildIndex();
  fs.writeFileSync(OUTPUT_FILE, JSON.stringify(resourceIndex, null, 2), 'utf-8');
  console.log(`Resource index written: ${resourceIndex.summary.totalResources} resources across ${resourceIndex.summary.totalFiles} files`);
} catch (err) {
